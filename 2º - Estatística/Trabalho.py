...
def formula():
    ...
    P_ate = 0
    while x_counter >= 0:
        P_ate += C(n,x_counter) * (p ** x_counter) * (q ** (n-x_counter))
        x_counter -= 1
    ...
    operações_tabulate.append([operação.id, operação.n, operação.x, operação.p_percent, operação.q_percent, operação.P_percent, operação.P_ate_percent, operação.P_pelomenos_percent])
...
def hist():
    ...
    print(tabulate(operações_tabulate, headers=cabeçalho, tablefmt='fancy_grid'))
