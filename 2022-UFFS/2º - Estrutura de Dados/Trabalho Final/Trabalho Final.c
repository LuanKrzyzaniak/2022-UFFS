...
void criarLista(Item *raizA, Item *raizB, Item * novaLista){
     if (raizA == NULL || raizB == NULL) return;

     Item *aux;
     criarLista(raizA->esquerdo, raizB, novaLista);
     criarLista(raizA, raizB->esquerdo, novaLista);
     if (strcmp(raizA->produto, raizB->produto) == 0){
          Item *novo;
          novo = malloc(sizeof(Item));
          strcpy(novo->produto, raizB->produto);
          novo->quantidade = 0;
          novo->direito = NULL;
          novo->esquerdo = NULL;
          if (novaLista == NULL){
               ...
               for (aux = novaLista; aux != NULL; aux = aux->direito){
                    if (aux->direito == NULL){
                         aux->direito = aux;
                    }
               }
     ...
     criarLista(raizA, raizB->direito, novaLista);
     criarLista(raizA->direito, raizB, novaLista);;
}
...
{
     // Criando uma nova árvore p/ fazer uma lista encadeada
     Item *listaNova = NULL;
     // Printar os produtos usando as árvores
     criarLista(raizA, raizB, listaNova);
